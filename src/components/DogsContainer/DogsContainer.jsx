    ...
    <div className="dogsContainer">
      <div className="filter_row ">
        <div className="filteredRow_item1 filterItems">
          <FilterAltIcon className="mainIcons" /> <span>Form to Release</span>
        </div>
        <div className="filteredRow_item1 filterItems">
          <PetsIcon className="mainIcons" onClick="{Form}" />{" "}
          <span>Form to Adopt</span>
          {/*<span>{doggosBreed ? doggosBreed : "Form to Adopt"}</span>*/}
        </div>
        <div className="filter_row_text filterItems">
          {" "}
      ...
      <div className="doggos_result">
        {loading ? (
          ...
          filteredData.map((item) => <DoggosImg key={item} image={item} />)
        ) : (
          doggosResult.map((item) => <DoggosImg key={item} image={item} />)
        )}
      </div>
    </div>
  );
